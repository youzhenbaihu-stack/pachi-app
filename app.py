...
# ページ設定
st.set_page_config(page_title="パチンコ回転率ツール", page_icon="🎰")

st.title("🎰 パチンコ回転率アナライザー")
st.write("グラフ画像（全色対応）と履歴画像（赤数字）をアップロードしてください。")

# --- 関数定義 ---
def analyze_graph(img):
    ...
    if not contours_line_graph: return None, "グラフ線なし"
...
def analyze_history(img):
    ...
    # OCR実行
    text = pytesseract.image_to_string(mask_inverted, config=r'--oem 3 --psm 6 outputbase digits')
...
if uploaded_graph:
    ...
    result, msg_or_img = analyze_graph(img_graph)
    
    if result is not None:
        diff_balls = result
        st.image(cv2.cvtColor(msg_or_img, cv2.COLOR_BGR2RGB), caption=f"解析結果: {diff_balls:+}発", use_column_width=True)
        st.success(f"推定差玉: {diff_balls} 発")
    else:
        st.error(f"エラー: {msg_or_img}")

st.subheader("② 履歴リストの解析（ST回転数）")
uploaded_history = st.file_uploader("履歴画像をアップロード（任意）", type=['jpg', 'png', 'jpeg'], key="history")
st_spins_auto = 0

if uploaded_history:
    file_bytes = np.asarray(bytearray(uploaded_history.read()), dtype=np.uint8)
    img_hist = cv2.imdecode(file_bytes, 1)
    st_sum, num_list = analyze_history(img_hist)
    st_spins_auto = st_sum
    st.info(f"検出された赤数字: {num_list}")
...
with col1:
    total_spins = st.number_input("現在の総回転数", min_value=0, value=3000)
with col2:
    st_spins_final = st.number_input("ST/時短の回転数", min_value=0, value=st_spins_auto)
...
with c3:
    ...
    payout_300 = st.number_input("チャージ 出玉/回", value=300)
...
if st.button("計算する", type="primary"):
    ...
    st.write(f"推定投資: **{int(used_balls):,}** 発 ({int(used_balls)*4:,} 円)")
    
    if used_balls > 0:
        rate = (real_spins / used_balls) * 250
        st.metric("1kあたりの回転数", f"{rate:.2f} 回転")
        if rate >= 20: st.balloons()
    else:
        st.error("投資がマイナスです")
